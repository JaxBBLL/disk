/**
 * 打包发布产物：把 .output、启动脚本、pm2 配置、config 收拢到 dist/，
 * 得到一个可直接拷贝到服务器运行的自包含目录。
 *
...
async function main() {
  if (!existsSync(join(root, ".output", "server", "index.mjs"))) {
    console.error(
      "[release] 未找到 .output/server/index.mjs，请先执行 nuxt build",
    );
    process.exit(1);
  }
  ...
  // 1. 服务端与静态资源（含运行时依赖 node_modules）
  await copyDeref(join(root, ".output"), join(dist, ".output"));

  // 2. 启动脚本
  await cp(join(root, "start.mjs"), join(dist, "start.mjs"));

  // 3. 生成 pm2 部署配置（部署相关文件只在构建产物中生成，开发目录不保留）
